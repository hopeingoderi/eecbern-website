(function(){
  ...
  if(mobileBtn && nav){
    ...
    document.addEventListener('click', (e)=>{
      ...
      const t = e.target;
      if(nav.contains(t) || mobileBtn.contains(t)) return;
      setMenu(false);
    });
  ...
  const defaultLang = savedLang || 'ti'; // default to Tigrinya for your community

  function applyLang(lang){
    const safe = (lang === 'ti') ? 'ti' : 'en';
    document.documentElement.setAttribute('data-lang', safe);
    document.documentElement.setAttribute('lang', safe === 'ti' ? 'ti' : 'en');
    try { localStorage.setItem('lang', safe); } catch(e){}
    langButtons.forEach(btn => btn.classList.toggle('is-active', btn.getAttribute('data-set-lang') === safe));
  }
